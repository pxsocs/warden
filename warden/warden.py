...
# Config Variables ---------------------------------------------------
# Known wallet folders - where json files are listed

home_path = str(Path.home())
known_wallet_folders = [
    home_path + '/.specter',  # pip install version
    '/mnt/hdd/mynode/specter'  # myNode installation
]
...
def specter_update(load=False):
    ...
    for wallet in specter_data.wallet_manager.wallets:
        ...
        for device in specter_data.wallet_manager.wallets[wallet].__dict__[
                'devices']:
            specter_data.wallet_manager.wallets[wallet].__dict__[
                'device_list'].append(device.alias)
...
class User:
    def __init__(self, name='mynode', fx='USD'):
        self.name = name
        self.fx = fx
...
def get_specter_tx(wallet_alias):
    ...
    # wallet.wallets.name.utxo
    # Import UTXOs into a dataframe
    for wallet in list_specter_wallets():
        for utxo in specter_data['wallets']['wallets'][wallet]['utxo']:
...
def specter_df(save_files=False):
    ...
    for alias in wallet_info:
        ...
        txs['trade_account'] = wallet_info[alias]['alias']
